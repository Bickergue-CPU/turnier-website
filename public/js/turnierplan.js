...
async function zeigeGruppeDetail(gruppeId, gruppeName) {
    ...
    try {
        const spieleRes = await fetch(`${API}/spiele?gruppe_id=${gruppeId}`);
        const spiele = await spieleRes.json();

        let html = `<h4>📅 Spielplan – ${gruppeName}</h4>`;

        if (!spiele || spiele.length === 0) {
            html += `<p>Noch keine Spiele vorhanden.</p>`;
        } else {
            html += `<div class="spielplan-liste">`;

            spiele.forEach(spiel => {
                const heim = spiel.heim_name || spiel.heim_id || '?';
                const gast = spiel.gast_name || spiel.gast_id || '?';
                const zeit = spiel.zeit || '--:--';
                ...
                if (spiel.tore_heim !== null && spiel.tore_gast !== null) {
                    ...
                    ergebnis = `<span class="ergebnis-offen">vs</span>`;
                }

                html += `
                    <div class="spiel-zeile">
                        <span class="spiel-zeit">${zeit}</span>
                        <span class="spiel-team heim">${heim}</span>
                        ${ergebnis}
                        <span class="spiel-team gast">${gast}</span>
                    </div>`;
            });
        ...
        detail.innerHTML = "<p>⚠️ Fehler beim Laden der Spiele.</p>";
    }
}
